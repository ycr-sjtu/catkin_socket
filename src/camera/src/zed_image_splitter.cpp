...
void zedImageCallback(const sensor_msgs::ImageConstPtr &msg)
{
    ...
    if (msg == nullptr)
        return;
    valid = true;
    // image_raw = cv_bridge::toCvShare(msg, "bgr8")->image.clone();
...
//        cv_bridge::CvImage image_bridge(header, sensor_msgs::image_encodings::BGR8, image_left);
//        cv_bridge::CvImage image_bridge(header, sensor_msgs::image_encodings::MONO8, image_gray);
